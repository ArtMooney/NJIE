...
document
  ...
  .addEventListener("input", function (event) {
    const inputList =
      event.target.parentElement.parentElement.querySelectorAll("input");
    const paraList =
...
document.getElementById("send-form").addEventListener("click", (event) => {
  ...
  const formId = document.getElementById("email-form");
  ...
  if (formComplete) {
    ...
    fetch("https://api.ngine.se/webhook/njiex-wp", requestOptions)
      ...
      .catch((error) => {
        ...
        formId.addEventListener(
          "click",
          (event) => {
            document.getElementById("send-button").innerHTML = "Skicka in";
